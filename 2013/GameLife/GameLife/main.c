 ...
 * Игра "жизнь", поле в форме тора
 * описание поля в файле input.txt (# - живая, все осталльное - мертвое, строки могут быть заполенны не до конца)
 */
#include <stdio.h>
#include <string.h>
#include <conio.h>
...
int di[STEPS] = {  0, -1, -1, -1,  0,  1,  1,  1};
int dj[STEPS] = {  1,  1,  0, -1, -1, -1,  0,  1};
...
int next_state(int i, int j) {
    int sum = 0, k, res;
    for (k = 0; k < STEPS; k++) {
        sum += ( map[( (i + ROWS + di[k] ) % ROWS ) * COLUMNS +
            (j + COLUMNS + dj[k] ) % COLUMNS ] != EMPTY ? 1 : 0);
    }
    if (map[i * COLUMNS + j] != EMPTY) {
        res = (sum > 3 || sum < 2) ? 0 : 1;    
    }
...
void next_frame() {
    ...
    for (i = 0; i < ROWS; i++) {
        for(j = 0; j < COLUMNS; j++) {
            buffer[i * COLUMNS + j] = (next_state(i, j) ? '#' : EMPTY);
        }
    }
}
