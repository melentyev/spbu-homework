...
int globalLineNumber = 1, globalLinePosition = 0;
...
{
    return globalCharBuf;
}

int nextChar(FILE *stream)
{
    globalCharBuf = getc(stream);
    if (globalCharBuf == '\n') 
    {
        globalLineNumber++;
        globalLinePosition = 0;
    }
    else 
    {
        globalLinePosition++;
...
CommandArg parseCommandArg(int allowRecursive)
{
    ...
    if (currentToken().type == TT_BRACKET_OPEN)
    {
        if (allowRecursive)
        {
            CommandArg *inner;
            nextToken();
            inner = (CommandArg*)ALLOCATE(sizeof(CommandArg) );
    ...
    else if (currentToken().type == TT_WORD)
    {
        ...
        if ( (reg = findRegister(currentToken().value.str) ) != -1)
        {
            ...
            res.argv.r = registers[reg].reg;
            FREE(currentToken().value.str);
        }
        else
        {
            res.type = AT_LABEL;
...
void releaseCommandArg(CommandArg arg) 
{
    if (arg.type == AT_ADDRESS)
    {
        FREE(arg.argv.addr);
    }
}
...
void releaseMemory() 
{
    ...
    for (i = 0; i < commandsCount; i++) 
    {
        ...
        case CT_JL: case CT_JNL:
        case CT_JG: case CT_JNG:
            releaseCommandArg(cmd->args.arg1);
            releaseCommandArg(cmd->args.arg2);
            break;
        default:
            releaseCommandArg(cmd->args.arg1);
            break;
